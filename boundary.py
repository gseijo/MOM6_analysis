...
def flood_missing(arr, **kwargs):
    ...
    import sys
    sys.path.append('/glade/work/gseijo/HCtFlood')
...
def standardize_in(data,temp_name,sal_name,u_name,v_name,zeta_name,lat_name,lon_name,depth_name,IC):
    if IC:
         nameZeta = 'ssh'
    else:
        nameZeta = 'zeta'
    ds_in = xarray.open_dataset(data).rename({lon_name: 'lon', 
                                           lat_name: 'lat',
                                           temp_name:'temp',
                                           sal_name: 'salt',
                                           u_name: 'u',
...
def data_standardize(file_path,file_name,variable_name,Lon_name,Lat_name,Levels_name,Time_name,positive_down,save_standard_format,time_resolution,data_tag):
    ds = xarray.open_dataset(file_path + file_name)
    ds=ds.rename({variable_name:'data',Lon_name:'lon',Lat_name:'lat',Time_name:'time',Levels_name:'z'})
    if not positive_down:
        ds['z']=ds['z']*-1
    #ds=ds.transpose('lat','lon','z','time')
    ds['data_tag'] = str(data_tag)
    ds['time_resolution'] = str(time_resolution)
    return ds

def obc_standardize(file_path,file_name,coords_in,variable_name,Lon_name,Lat_name,Levels_name,Time_name,positive_down,save_standard_format,time_resolution,data_tag):
        dsc = xarray.open_dataset(coords_in,engine='netcdf4')
        ds = xarray.open_dataset(file_path + file_name)
        ds=ds.rename({variable_name:'data',Lon_name:'lon',Lat_name:'lat',Time_name:'time',Levels_name:'z'})
        ds=ds.rename({'z':'dz'})
        ds['z']=ds['dz'][0,:,0,0]
        for i in range(len(ds['dz'][0,:,0,0])):
               if i == 0:
                    ds['z'][i] = ds['dz'][0,i,0,0].values
               else:
                    ds['z'][i] = ds['dz'][0,i,0,0].values + ds['z'][i-1]
        ds
        lats = dsc['lat'].values
        lons = dsc['lon'][0:1].values
        datas = ds['data'][:,:,:,:]
        print(np.shape(datas))
        ds_new = xarray.Dataset(
            data_vars=dict(
                data=(["time", "z","lat","lon"], datas)),
            coords=dict(
                lon=(["lon"], lons),
                lat=(["lat"], lats),
                time=ds['time'],
